...
const main = async () => {
  ...
  // Reset the role_id sequence to 1 after deleting the data.
  // This ensures the next role created will have an id of 1.
  ...
  // Users
  const hashedPassword = await bcrypt.hash('userP@$$', 10);
  const user1 = await prisma.users.create({
    data: {
      name: 'John Doe',
      email: 'john.doe@playverse.com',
      address: "123 Play Street, Boon Keng, Singapore",
      phone_number: "+6590809090",
  ...
  const user2 = await prisma.users.create({
    data: {
      ...
      address: "234 George Street, Queenstown, Singapore",
      phone_number: "+6590909090",
      password_hash: await bcrypt.hash('@dminP@$$', 10),
